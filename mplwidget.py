...
class MplCanvas(FigureCanvas):  # 小部件的画布（后台画布）
    ...
    def __init__(self, nrows=2, ncols=2):
        self.fig, self.ax = plt.subplots(nrows=nrows, ncols=ncols)  # 建立图像区域
...
class MplWidget(QWidget):  # mpl小部件（全部小部件）；；要添加的类名字
    ...
    def __init__(self, parent=None, nrows=2, ncols=2):
        QWidget.__init__(self, parent)  # 继承小部件的所有初始化条件
        # 建立小部件画布
        self.canvas = MplCanvas(nrows=nrows, ncols=ncols)
        # 为画布创建一个导航条，创建给窗口自己
